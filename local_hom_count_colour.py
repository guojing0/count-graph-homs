...
def count_homomorphisms_best_colour(graph, target_graph):
    ...
    EXAMPLES::
        ...
        sage: from sage.graphs.hom_count_best_colour import count_homomorphisms_best_colour
        sage: count_homomorphisms_best_colour(graph, target_graph)
        324
    """
    ...
    for node in reversed(dir_labelled_TD.vertices()):
        ...
        match node_type:
            ...
            case _: 
                _add_leaf_node_best_colour(DP_table, node)

    return DP_table[0][0]

### Main adding functions

def _add_leaf_node_best_colour(DP_table, node):
    r"""
...
def _add_join_node_best_colour(DP_table, node, graph_TD):
    node_index, node_vertices = node
